/*
  SQL/13_crear_base_produccion_limpia.sql
  Creación de base LIMPIA para PRODUCCIÓN — SistemaCursosProduccion

  ═══════════════════════════════════════════════════════════════════════════════
  PROPÓSITO
  ═══════════════════════════════════════════════════════════════════════════════
  Crear (o recrear) la base SistemaCursosProduccion con la estructura final
  del proyecto, sin datos de prueba y SIN la tabla legacy RecursosCurso.

  Este script consolida la estructura equivalente a ejecutar en producción:
    01, 02, 03, 04, 05, 06, 07, 09

  NO incluye ni ejecuta:
    08_asignar_admin_prueba.sql   (solo desarrollo)
    10_crear_recursos_curso.sql   (legacy — RecursosCurso retirado del sistema)
    11_preparar_base_produccion.sql (limpia datos; no aplica si se crea desde cero)
    12_eliminar_recursos_generales.sql (no necesario: RecursosCurso nunca se crea aquí)

  ═══════════════════════════════════════════════════════════════════════════════
  ADVERTENCIAS CRÍTICAS
  ═══════════════════════════════════════════════════════════════════════════════
  • Ejecutar ÚNICAMENTE en el servidor SQL donde desea la base de PRODUCCIÓN.
  • NUNCA ejecutar sobre SistemaCursos (desarrollo).
  • NO ejecutar automáticamente desde la aplicación Node.js.
  • Si SistemaCursosProduccion ya tiene tablas, el script se detiene salvo que
    cambie @ConfirmarRecreacion = 1 (ELIMINA TODOS LOS DATOS y recrea estructura).
  • Después de ejecutar: registrar usuarios reales desde la plataforma web.
  • Luego asignar rol administrador al usuario real por SQL (ver sección al final).

  ═══════════════════════════════════════════════════════════════════════════════
  CÓMO USAR (PASOS MANUALES — NO AUTOMÁTICOS)
  ═══════════════════════════════════════════════════════════════════════════════
  1. Abrir SSMS conectado al servidor de PRODUCCIÓN (no al entorno de desarrollo
     si comparten instancia, verifique que apunta al servidor correcto).
  2. Confirmar que SistemaCursos (desarrollo) NO será la base activa al ejecutar.
  3. Abrir este archivo y revisar @ConfirmarRecreacion (dejar en 0 si es primera vez).
  4. Ejecutar el script completo (F5).
  5. Revisar las consultas de verificación al final:
       - roles = 5
       - usuarios = 0
       - categorías = 5
       - cursos, inscripciones, certificados, intentos = 0
       - RecursosCurso NO debe existir
  6. Levantar la app apuntando .env a DB_DATABASE=SistemaCursosProduccion (cuando corresponda).
  7. Registrar usuarios reales (instructores con @itq.edu.ec, usuarios externos, etc.).
  8. Asignar administrador al correo real (UPDATE al final comentado).

  ═══════════════════════════════════════════════════════════════════════════════
  ALTERNATIVA AL SCRIPT 11
  ═══════════════════════════════════════════════════════════════════════════════
  Si ya restauró un .bak de desarrollo como SistemaCursosProduccion y solo quiere
  limpiar datos conservando cursos reales, use SQL/11_preparar_base_produccion.sql
  en lugar de este script 13.

  Use el script 13 cuando quiera una base vacía desde cero (recomendado para go-live).
*/

/* ── 1. Crear base de datos si no existe ── */
IF DB_ID('SistemaCursosProduccion') IS NULL
BEGIN
    CREATE DATABASE SistemaCursosProduccion;
    PRINT 'Base SistemaCursosProduccion creada.';
END
ELSE
BEGIN
    PRINT 'La base SistemaCursosProduccion ya existe.';
END;
GO
...
/* ── 2. Protección: nunca sobre desarrollo ── */
...
BEGIN
    ...
    BEGIN
        RAISERROR(
            'SistemaCursosProduccion ya contiene tablas. Para recrear desde cero cambie @ConfirmarRecreacion = 1 (ELIMINA TODOS LOS DATOS).',
            16,
            1
        );
        RETURN;
...
/* ── Usuarios (vacía — sin cuentas @test.com) ── */
...
/*
  NO se insertan usuarios de prueba:
    - admin@test.com
    - instructor@test.com
    - estudiante@test.com

  Después del go-live, asignar administrador a un usuario REAL registrado en la web:

  UPDATE u
  SET u.id_rol = r.id_rol
  FROM Usuarios u
  INNER JOIN Roles r ON r.nombre_rol = 'administrador'
  WHERE u.correo = 'correo.real@itq.edu.ec';
*/
...
PRINT '';
PRINT '=== VERIFICACION SistemaCursosProduccion ===';
...
SELECT
    CASE
        ...
        ELSE 'ERROR — RecursosCurso aun existe (no deberia en produccion limpia)'
...
PRINT 'Script 13 finalizado. Revise que total_usuarios = 0 y RecursosCurso no exista.';
GO
